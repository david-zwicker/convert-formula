...
class ParserText(object):
    ...
    default_cost = 10.   #< default cost for unknown functions
    costs = { 'UNARY-' : 0.,
              'array' : 0.,
              '-' : 1.,
              '+' : 1.,
    ...
    optimize_threshold = 5. #< least saving to actually perform an optimization
    ...
    def _costs_subexpressions_rec(self, token, costs, counter):
        ...
        if isinstance(token, dict):
            ...
            for t in token['args']:
                costs, counter = \
                        self._costs_subexpressions_rec(t, costs, counter)
    ...
    def _costs_subexpressions(self, lines):
        ...
        for line in lines:
            costs, counter = \
                        self._costs_subexpressions_rec(line, costs, counter)
        
        # remove all subexpressions which only occur once
        for hash_id, count in counter.iteritems():
            if count < 2:
                del costs[hash_id]
    ...
    def _replace_subexpressions(
            self, token, hash_replace, hash_token, replacement
       ):
    ...
    def _optimize_once(self, lines):
        ...
        res.insert(
            first_line,
            {'op': '=', 'pos': 'infix', 'args': [temp_var, hash_token]}
       )
